...
function parseFrontmatterFull(content: string): { data: Record<string, unknown>; body: string } {
...
function loadSkill(dirPath: string): SkillEntry | null {
  ...
  const userInvocable = (data['user-invocable'] as string) === 'true';
...
function loadRule(dirPath: string, langDir: string): RuleEntry | null {
  ...
  if (lastModified === 0) lastModified = getMtime(dirPath);
...
function getSkillsData(): SkillsResponse {
  ...
  for (const langDir of rulesLangDirs) {
    ...
    if (rule) combined.push(rule as unknown as CombinedEntry);
  }
