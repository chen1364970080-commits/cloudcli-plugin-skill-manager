...
function filterBadge(filter, active, count, c) {
...
function skillCard(s, c) {
    ...
    if (isSkill && (s.whenToUse || s.paths?.length || s.model || s.hooks)) {
        const details = [];
        if (s.whenToUse)
            details.push(`<div style="margin-bottom:6px"><span style="font-size:0.62rem;color:${c.muted}">when</span><div style="font-size:0.68rem;line-height:1.4;opacity:0.8">${truncate(s.whenToUse, 160)}</div></div>`);
        if (s.model)
            details.push(`<div style="font-size:0.65rem;color:${c.muted}">model: <span style="color:${c.text};opacity:0.7">${s.model}</span></div>`);
        if (s.paths?.length)
            details.push(`<div style="font-size:0.65rem;color:${c.muted}">paths: <span style="color:${c.text};opacity:0.7">${s.paths.slice(0, 3).join(', ')}${s.paths.length > 3 ? '…' : ''}</span></div>`);
        if (s.hooks && Object.keys(s.hooks).length > 0)
            details.push(`<div style="font-size:0.65rem;color:${c.muted}">hooks: <span style="color:${c.text};opacity:0.7">${Object.keys(s.hooks).join(', ')}</span></div>`);
        if (details.length) {
...
function ensureStyles() {
    if (document.getElementById('skm-styles'))
        return;
...
let searchQuery = '';
function render(root, data, loading, error, ctx) {
    ...
    if (data) {
        filtered = data.skills.filter((s) => {
            if (currentFilter === 'skills' && s.type !== 'skill')
                return false;
            if (currentFilter === 'rules' && s.type !== 'rule')
                return false;
            if (searchQuery) {
  ...
  `;
    // Wire search input
    const searchInput = root.querySelector('#skm-search');
    if (searchInput) {
    ...
    // Wire filter buttons
    for (const btn of Array.from(root.querySelectorAll('[data-filter]'))) {
